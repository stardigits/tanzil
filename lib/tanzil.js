...
Tanzil.prototype.start = function (){
  ...
  function generateIndex() {
    ...
    var tf = options.pathTrans + '/' + options.transFile;
    var idxf = options.pathTrans + '/' + options.indexFile;
    var globalOk = false;
    var localOk = fs.existsSync(tf); 
    if (!localOk) if (/^win/.test(process.platform)) {
      var tf = options.pathApp + '/' + options.pathTrans + '/' + options.transFile;
      var idxf = options.pathApp + '/' + options.pathTrans + '/' + options.indexFile;
      globalOk = fs.existsSync(tf);
    }
    if (localOk||globalOk) {
  ...
  function getTransByAya(aya) {
    ...
    } else console.log(options.transFile + ' is not found in local or global app path');
    return trans;
  }
  ...
  function searchTrans(query) {
    ...
    if (!localOk) if (/^win/.test(process.platform)) {
      var tf = options.pathApp + '/' + options.pathTrans + '/' + options.transFile;
      var idxf = options.pathApp + '/' + options.pathTrans + '/' + options.idxFile;
      globalOk = fs.existsSync(tf);
    }
    if (localOk||globalOk) {
      var arr = getTrans(tf);
